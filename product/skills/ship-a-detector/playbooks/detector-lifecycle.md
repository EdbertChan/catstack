...
kind per step, each with the failure that paid for it. Steps 11–13 are the
...
each new shape was described rather than pasted. One widening pasted one
real reply — "My mistake — the skill does have disable-model-invocation:
true (I misread it)" — and it scored zero hits against every pattern the
hook had. The next went further and counted the shape across the whole
transcript corpus: 53 replies opened with that concession, 13 of them
straight after a human correction, and none matched.
`gh-write-verification` was built as four detectors off three sessions that
all hit the same broken `gh pr edit` in one day.
...
wait. A later widening of the same detector opens its Non-goals with
"Does not change the silent set. Every previously-allowed shape still has a
passing negative fixture." Its detector 2 is an allowlist of
...
`check_hook_test_coverage.py`, where it fails a build, rather than in prose
asking authors to remember.
...
The `--repo` fix called itself the "second, independent instance" of a
class already fixed once. Do the enumeration in one pass rather
...
- **Fenced blocks.** `diu-stop` counted a fenced YAML plan as prose and
  ...
  real closing fence when you strip, or an unterminated ` ``` ` becomes a way to smuggle unlimited text past the
  gate.
...
- the one-shot form of a shape you only caught in a loop. One widening exempted
  a bare `pgrep -f` as a legitimate "is it running?" check; the next tested
  that assumption three days later and it was false — the harness runs every
...
exemption rather than assuming it (that is exactly what the `pgrep -f`
exemption cost).
...
change — "cut 40 words: drop a section" — not "shorten it". The fix's own
Review Claim paired the two: the `stop_hook_active` return **and** the
reason naming how many words to cut. A later widening's `decide()`
short-circuits on
...
Then pin the interaction. The follow-up added a fix/re-trigger matrix: for each
...
small, one per repo, **outside the worktree**, with a TTL (`pr-schema-gate`
uses two hours). Every state read that cannot be understood means "nothing
owed": missing, unreadable, malformed, wrong-typed, future-dated and expired all
read as nothing owed, and an unwritable state directory logs to stderr and
allows. Arm state on the real action only, never on a rehearsal (step 5).
...
`check_hook_test_coverage.py` deliberately never asserts which direction you
must pick — only that you tested the one you picked.
...
fix went as far as landing the failing regression test as its own PR before
the fix itself landed. Another ran each suite twice — once with only the
...
- **No dated provenance lines.** No "as of <date>", no incident narrative,
  no PR numbers or commit SHAs in the rule text.
  `scripts/check_no_dated_provenance.py` is the gate.
...
One fix's first line: "Two shipped hooks never ran. install.sh symlinked them
...
the repo and both dead. A later PR hit the identical failure again with
...
merged settings. The `gh-write-verification` block is the shape to copy — it
asserts the link target, then `PreToolUse`, `Stop`, and `SubagentStop` each contain the
right command.

A class-level test checks that every `claude_*.py` entrypoint under
...
The README line lands in the **same** PR as the pattern, not after. One
widening shipped "two new admission patterns, two new negative guards, plus
their tests and the README line documenting them" as one slice; the two
`gh-write-verification` widenings each carried their own README section in
the diff.
...
got theirs in a later PR: five PRs each shipped a hook with no row, and a
sixth came back afterwards to add all of them at once. The right way is
what one later hook did — the hook and its row landed in the same commit.
...
it fires. A `wrong-check-reflect` widening added one trigger line to
`reflect`'s invoke list alongside
...
say which ran and which did not. One Test Plan ran each suite twice and
pasted both; another ran "every `engine/hooks/*/tests` directory plus
