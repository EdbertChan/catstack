...
straight after a human correction, and none matched. `gh-write-verification`
was built as four detectors off three sessions that all hit the same broken
`gh pr edit`.
...
passing negative fixture." Its detector 2 is an allowlist of state-changing
commands rather than a blocklist of safe ones, so every read-only command
with discarded output is silent by construction, not by exclusion rule.
...
class already fixed once. Do the enumeration in one pass rather than paying
for a third. When the resolver cannot resolve — the flag is
...
- **Fenced blocks.** `diu-stop` counted a fenced YAML plan as prose and
  blocked a legitimate reply, which lost a staged plan draft. Require a real closing fence when you strip, or an
...
- the one-shot form of a shape you only caught in a loop. One change
  exempted a bare `pgrep -f` as a legitimate "is it running?" check; the
  next tested that assumption and it was false — the harness runs every
...
exemption rather than assuming it; an untested exemption costs a second PR.
...
words to cut. A later widening's `decide()` short-circuits on
...
small, one per repo, **outside the worktree**, with a TTL — two hours
works. Every state read that cannot be understood means "nothing owed":
...
allows. Arm state on the real action only, never on a rehearsal
(step 5).
...
"allowing", so a detector bug can only under-block. `pr-schema-gate` fails
open when a `--repo` flag names a repo with no matching checkout. The
unreadable-input gate deliberately never asserts which direction you must
...
One repair opened with: "Two shipped hooks never ran. install.sh symlinked
them into the hooks dir but no installer merged their commands into
...
got theirs in a later PR: five PRs each shipped a hook with no row, and one
follow-up came back afterwards to add all of them at once. The right shape
is the hook and its row landing in the same commit.
...
it fires. One widening added one trigger line to `reflect`'s invoke list
alongside the pattern, and says why: "The reflect skill's invoke list names the same
...
say which ran and which did not. A good Test Plan runs each suite twice
and pastes both, or runs "every `engine/hooks/*/tests` directory plus
