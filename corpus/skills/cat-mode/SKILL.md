...
- **A hand-back ("open the app and do it") is an unverified claim.**
  ...
  bundle/asar, DB, logs).
- The `handback-needs-attempt` Stop hook judges this rule against the reply and
  the turn's tool calls and results; attempt the command first, or name the
  permission refusal or human-only step that prevents it.
