...
- **External data is untyped until parsed.** RPC payloads, JSON, IPC messages, CLI args, config files, environment variables, database rows. Have a parse function at every boundary that turns unstructured input into the typed model. See the **boundary-discipline** principle skill for where to put validation. Error, log, and exit text is external data too: branch on the recorded state behind a failure, per cat-mode's "Categorical constraints & recurrence" rule on error text.
...
**Battle-tested:** A `completed` task record has `execution: { branch?: string }`. Completion logic assumes `branch` is always set once `status === 'completed'` and throws a runtime guard when it isn't — this fired unpredictably in production after restarts or partial writes. The type admits `status: 'completed', branch: undefined`, which is meaningless. Fix at the type: `{ status: 'pending' } | { status: 'completed'; execution: { branch: string, ... } }`, so a completed record cannot compile without a branch — and delete the runtime throw once the type makes it impossible, don't leave both. A team that hits this exact tension and chooses "keep the guard, it catches real integrity issues" over fixing the type has diagnosed the disease and treated the symptom.

**Grep for it:** search for `throw`/`assert`/`invariant` strings containing phrases like "should never happen," "completed without," "missing," "expected X to be" that follow an `if (x.status === '<terminal-value>' && !x.<field>)` shape. Each hit is a candidate for a discriminated-union refactor — this pattern is portable static analysis in any typed codebase, not project-specific.
