...
2. **Name its omission twin in the next line.** Every "must not invent"
   rule gets a "must not be missing" rule; how a hard row ships is
...
5. **Re-validate** (re-run the grading/review/test suite) before claiming
   fixed. The proof standard already exists and is not restated here:
   "Repro then fix: fail-before and pass-after, both outputs pasted"
   (`engine/CLAUDE.core.md`, Named constraints), and [[prove-it-ship-gate]]
   for any done/shipped claim with live side effects. A fix isn't proven
   until the same class of check runs clean again, not just the one case
