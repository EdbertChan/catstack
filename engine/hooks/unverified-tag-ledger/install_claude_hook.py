...
Each entry in HOOK_SPECS is (hook type in settings.json, marker identifying this
hook's own entry, fragment file). The marker is directory-qualified: diu-stop's
scripts carry the same basenames, so a bare "claude_stop_check.py" marker would
match diu-stop's entries and delete them instead of this hook's.
