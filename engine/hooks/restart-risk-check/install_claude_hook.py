#!/usr/bin/env python3
"""Merge restart-risk-check into ~/.claude/settings.json Stop hooks. Idempotent."""
...
MARKER = "restart-risk-check/claude_stop_restart_check.py"
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude Stop restart-risk-check already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("added   claude Stop restart-risk-check")
