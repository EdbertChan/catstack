...
The shared judge reports an `unchecked` result explicitly when no judge runner
answers; it is never treated as clean. `stop_hook_active` returns immediately
so a rewritten reply can finish.
