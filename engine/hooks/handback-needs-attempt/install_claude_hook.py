...
def merge_hook(settings, fragment):
    entries = settings.setdefault("hooks", {}).setdefault("Stop", [])
    before = json.dumps(entries, sort_keys=True)
    entries[:] = [entry for entry in entries if not any(MARKER in hook.get("command", "") for hook in entry.get("hooks", []))]
    entries.extend(fragment.get("hooks", {}).get("Stop", []))
