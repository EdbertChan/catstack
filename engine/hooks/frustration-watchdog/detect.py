...
RULE_NO_VISIBLE_HANDOFF = "frustration-watchdog.no-visible-handoff"
...
def detect(event):
    ...
    return [
        Finding(
            rule_id=RULE_NO_VISIBLE_HANDOFF,
            subject=_reply_subject(message),
            message=_message(sorted(set(kinds)), refused, unchecked),
...
def _reply_subject(message):
    return "reply:" + hashlib.sha256(message.encode("utf-8")).hexdigest()


def _message(kinds, refused, unchecked):
    head = (
        f"The user's last message was impatience-shaped ({', '.join(kinds)}) "
