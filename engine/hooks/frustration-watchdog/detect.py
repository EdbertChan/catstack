...
RULE_NO_VISIBLE_NEXT_STEP = "frustration-watchdog.no-visible-next-step"
RULE_REFUSAL_NO_VISIBLE_NEXT_STEP = "frustration-watchdog.refusal-no-visible-next-step"
RULE_UNCHECKED_REFUSAL_STATE = "frustration-watchdog.unchecked-refusal-state"
...
def detect(event: dict[str, object]) -> list[Finding]:
    ...
    if refused:
        ...
        rule_id = RULE_REFUSAL_NO_VISIBLE_NEXT_STEP
    else:
        message_text = (
            head
        ...
        rule_id = RULE_NO_VISIBLE_NEXT_STEP
    if unchecked:
        message_text += (
            f"\n(frustration-watchdog could not read this turn's tool results ({unchecked}), "
            "so it could not tell whether a hook refused a tool call; the wording above "
            "is the default.)"
        )
        rule_id = RULE_UNCHECKED_REFUSAL_STATE
    return [
        Finding(
            rule_id=rule_id,
            subject=_subject(message, msgs[-1][1], kinds),
...
def _subject(reply: object, user_text: str, kinds: list[str]) -> str:
    basis = "\n".join([str(reply), user_text, ",".join(sorted(set(kinds)))])
    return f"reply:{hashlib.sha256(basis.encode('utf-8')).hexdigest()}"
