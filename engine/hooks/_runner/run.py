...
def _create_findings_file() -> str:
    with tempfile.NamedTemporaryFile(
        "w",
        encoding="utf-8",
        prefix="catstack-hook-findings-",
        suffix=".json",
        delete=False,
    ) as handle:
        json.dump([], handle)
...
def _read_rule_ids(path: str) -> tuple[list[str], bytes]:
    ...
    if not isinstance(payload, list) or any(not isinstance(rule_id, str) for rule_id in payload):
        return [], f"catstack-hook-error runner: invalid findings file {path}: expected JSON array of strings\n".encode()
    return payload, b""


def _delete_findings_file(path: str) -> bytes:
    try:
        os.unlink(path)
    except FileNotFoundError:
        return b""
    except OSError as exc:
        return f"catstack-hook-error runner: could not delete findings file {path}: {exc}\n".encode()
    return b""
...
def main(argv: list[str] | None = None) -> int:
    ...
    stderr = b""
    findings_error = b""
    findings_path = _create_findings_file()
    rule_ids: list[str] = []
    exit_code = 1
    timed_out = False
    ...
    try:
        ...
        findings_error += _delete_findings_file(findings_path)
