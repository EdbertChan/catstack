...
Flag matching carries no trailing boundary, because a value may be attached to
its flag: `-S tok`, `-S"tok"` and `-Stok` are the same search and all three
count as having run. Matching is case-sensitive for the same reason it has to
be — `gh pr list --search` must not read as `git log -S`.
...
- A search reported only as something that was *asked for* rather than done —
  "the checklist told me to run `git log -S`". Reported speech about an
  instruction is not a report that the search ran, and across 14,994 local
  sessions this shape was 12 of the 13 fires before it was excluded.
...
Sentences are split on newlines as well as on end punctuation, so a sentence
soft-wrapped across two lines is read as two. When the instruction verb lands
on the first line and the backticked command on the second, the exclusion
above cannot see it and the hook fires. That is 1 fire in 15,010 local
sessions. It is advisory, so the cost is one stderr line.
