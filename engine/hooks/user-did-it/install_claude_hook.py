#!/usr/bin/env python3
"""Merge user-did-it into ~/.claude/settings.json UserPromptSubmit hooks. Idempotent."""
...
MARKER = "user-did-it/claude_prompt_submit.py"
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude UserPromptSubmit user-did-it already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("added   claude UserPromptSubmit user-did-it")
