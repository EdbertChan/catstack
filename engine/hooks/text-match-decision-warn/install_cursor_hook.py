#!/usr/bin/env python3
"""Idempotently merge text-match-decision-warn into Cursor hooks.json."""
...
FRAGMENT = {
    "preToolUse": [{
        "matcher": "*",
        "command": "python3 $HOME/.cursor/hooks/text-match-decision-warn/cursor_pretooluse.py",
        "timeout": 5,
    }],
}
...
def main() -> None:
    ...
    if merged == data:
        print("ok      cursor text-match-decision-warn hooks already up to date")
        return
    os.makedirs(os.path.dirname(HOOKS_PATH), exist_ok=True)
    with open(HOOKS_PATH, "w", encoding="utf-8") as handle:
        json.dump(merged, handle, indent=2)
        handle.write("\n")
    print("link    cursor preToolUse text-match-decision-warn merged")
