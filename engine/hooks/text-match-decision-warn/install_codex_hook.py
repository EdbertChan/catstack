#!/usr/bin/env python3
"""Idempotently merge text-match-decision-warn into Codex's native hooks (~/.codex/hooks.json)."""
...
MARKERS = {"PreToolUse": "text-match-decision-warn/codex_pretooluse.py"}
...
def main() -> None:
    ...
    if merged == settings:
        print("ok      codex text-match-decision-warn hooks already up to date")
        return
    os.makedirs(os.path.dirname(HOOKS_PATH), exist_ok=True)
    with open(HOOKS_PATH, "w", encoding="utf-8") as handle:
        json.dump(merged, handle, indent=2)
        handle.write("\n")
    print("link    codex PreToolUse text-match-decision-warn merged")
