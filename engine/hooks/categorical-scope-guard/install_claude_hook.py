#!/usr/bin/env python3
"""Idempotently merge categorical-scope-guard into Claude settings."""
...
MARKERS = {
    "PreToolUse": "categorical-scope-guard/claude_pretooluse.py",
}
...
def main() -> None:
    ...
    if merged == settings:
        print("ok      claude categorical-scope-guard hooks already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w", encoding="utf-8") as handle:
        json.dump(merged, handle, indent=2)
        handle.write("\n")
    print("link    claude PreToolUse categorical-scope-guard merged")
