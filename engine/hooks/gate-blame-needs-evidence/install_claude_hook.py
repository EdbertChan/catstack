#!/usr/bin/env python3
"""Merge gate-blame-needs-evidence into ~/.claude/settings.json (Stop).
Idempotent."""
...
MARKER = "gate-blame-needs-evidence/"
EVENTS = ("Stop",)
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude Stop gate-blame-needs-evidence already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("added   claude Stop gate-blame-needs-evidence")
