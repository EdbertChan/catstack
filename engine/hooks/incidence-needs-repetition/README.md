...
On every Stop, `detect.py` hands the latest assistant reply to the background judge using [`engine/hooks/llm-judge/phrases/incidence-needs-repetition.json`](../llm-judge/phrases/incidence-needs-repetition.json). The answer arrives on a later turn through the shared [`llm-judge`](../llm-judge/README.md) inbox, carrying the dictionary's `on_hit` follow-up text.

The live reply is never held up. If the result could not be checked, the inbox says "could not judge" instead of treating the reply as clean. A clean verdict says nothing. A turn that already repeated the same measurement stays exempt.

No job is sent when `stop_hook_active` is set, when the reply is empty, or when transcript state cannot be read. All enqueue errors fail open.

To grow the dictionary, add the real text of any miss to `match`, or the real text of any false alarm to `not_match`. Do not add a pattern; the prose meaning belongs in the phrase dictionary.
