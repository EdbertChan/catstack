#!/usr/bin/env python3
"""Merge incidence-needs-repetition into ~/.claude/settings.json Stop hooks. Idempotent."""
...
MARKER = "incidence-needs-repetition/claude_stop_check.py"
EVENT = "Stop"
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude Stop incidence-needs-repetition already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("added   claude Stop incidence-needs-repetition")
