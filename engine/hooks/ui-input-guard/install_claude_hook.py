#!/usr/bin/env python3
"""Merge ui-input-guard into ~/.claude/settings.json PreToolUse hooks. Idempotent."""
...
MARKER = "ui-input-guard/claude_pretooluse_check.py"
EVENT = "PreToolUse"
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude PreToolUse ui-input-guard already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("added   claude PreToolUse ui-input-guard")
