#!/usr/bin/env python3
"""SDK mode and event-row tests for ui-input-guard."""
...
HOOK = HOOK_DIR / "claude_pretooluse_check.py"
RULE_ID = "ui-input-guard.applescript-system-events-input"


def bash_payload(registry_path: Path, session_id: str = "ui-input-guard-sdk-test") -> dict[str, object]:
    return {
        ...
        "tool_name": "Bash",
        "tool_input": {
            "command": "osascript -e 'tell application \"System Events\" to keystroke \"3\"'",
        },
    }
...
class UiInputGuardSdkModeTest(unittest.TestCase):
    def test_warn_override_changes_registry_stop_response_to_warning(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stop_env.pop("CATSTACK_HOOK_MODE_UI_INPUT_GUARD", None)
            ...
            warn_env.update(
                {
                    ...
                    "CATSTACK_HOOK_MODE_UI_INPUT_GUARD": "warn",
                }
            )
        ...
        self.assertIn("ui-input-guard", stop_result.stderr)
        ...
        self.assertIn("ui-input-guard", warn_body["hookSpecificOutput"]["additionalContext"])
    ...
    def test_each_finding_writes_one_event_row_with_rule_id(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            env.pop("CATSTACK_HOOK_MODE_UI_INPUT_GUARD", None)

            result = run_hook(bash_payload(registry_path, "ui-input-guard-event-row"), env)
            rows = self._event_rows(metrics)

        self.assertEqual(2, result.returncode, result.stdout)
        self.assertEqual(1, len(rows))
        self.assertEqual("ui-input-guard", rows[0]["hook"])
        ...
        path.write_text(
            "\n".join(
                [
                    ...
                    f'mode = "{mode}"',
                    'why_mode = "outward"',
                    'summary = "Stops driving the user keyboard or screen."',
                    "",
                ]
            ),
            encoding="utf-8",
