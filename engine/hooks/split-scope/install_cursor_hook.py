#!/usr/bin/env python3
"""Merge split-scope Cursor hooks into ~/.cursor/hooks.json without wiping others."""
...
FRAGMENT = {
    "beforeSubmitPrompt": [
        {
            "command": "python3 $HOME/.cursor/hooks/split-scope/cursor_before_submit.py",
            "timeout": 10,
        }
    ],
    "postToolUse": [
        {
            "command": "python3 $HOME/.cursor/hooks/split-scope/cursor_post_tool_use.py",
            "timeout": 5,
        }
    ],
}
...
MARKERS = {
    "beforeSubmitPrompt": "split-scope/cursor_before_submit.py",
    "postToolUse": "split-scope/cursor_post_tool_use.py",
}
...
def main() -> None:
    ...
    for key, incoming in FRAGMENT.items():
        ...
        if before != after:
            changed = True
            print(f"link    cursor {key} split-scope merged")
        else:
            print(f"ok      cursor {key} split-scope already up to date")

    stop = list(hooks.get("stop") or [])
    if not any("Apply diu" in str(entry.get("prompt", "")) for entry in stop):
