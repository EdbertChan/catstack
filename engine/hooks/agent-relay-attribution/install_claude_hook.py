#!/usr/bin/env python3
"""Merge agent-relay-attribution into ~/.claude/settings.json Stop hooks. Idempotent."""
...
MARKER = "agent-relay-attribution/claude_stop_check.py"
EVENT = "Stop"
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude Stop agent-relay-attribution already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("added   claude Stop agent-relay-attribution")
