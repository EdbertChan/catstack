#!/usr/bin/env python3
"""Merge history-claim-check's Claude PreToolUse hook into ~/.claude/settings.json without wiping others."""
...
FRAGMENT_PATH = os.path.join(HERE, "claude.tool.hook.json")
MARKER = "history-claim-check/claude_pretooluse.py"
...
def main() -> None:
    ...
    if before == after:
        print("ok      claude PreToolUse history-claim-check already up to date")
        return
    ...
    with open(SETTINGS_PATH, "w") as f:
        ...
        f.write("\n")
    print("link    claude PreToolUse history-claim-check merged")
