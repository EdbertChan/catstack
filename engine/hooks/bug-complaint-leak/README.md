...
Fail-open. Registry mode is `warn`. Inject-only on prompt submit (does not run
git). Does not vendor `skills/reflect/` into Invoker. Does not fire on ordinary
implement prompts like "add a comment to Foo.ts".

## Files

- `detect.py` — bug-complaint matching, checklist text, and SDK findings
- `state.py` — session cache for empty/repeat Grep
- `claude_prompt_submit.py` — Claude `UserPromptSubmit` inject
- `claude_pretooluse_grep.py` / `claude_posttooluse.py` — Claude Grep leak gate
- `cursor_before_submit.py` / `cursor_pretooluse_grep.py` / `cursor_post_tool_use.py` — Cursor parity
...
```sh
python3 -m unittest discover -s engine/hooks/bug-complaint-leak/tests -v
```
