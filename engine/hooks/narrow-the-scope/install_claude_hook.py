#!/usr/bin/env python3
"""Merge narrow-the-scope into ~/.claude/settings.json PostToolUse hooks. Idempotent."""
...
MARKER = "narrow-the-scope/claude_posttooluse.py"
EVENT = "PostToolUse"
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude PostToolUse narrow-the-scope already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("added   claude PostToolUse narrow-the-scope")
