#!/usr/bin/env python3
"""Merge answer-overrides-menu into ~/.claude/settings.json PostToolUse hooks. Idempotent."""
...
MARKER = "answer-overrides-menu/claude_posttooluse.py"
EVENT = "PostToolUse"
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude PostToolUse answer-overrides-menu already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("added   claude PostToolUse answer-overrides-menu")
