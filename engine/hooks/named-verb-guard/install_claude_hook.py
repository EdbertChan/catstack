#!/usr/bin/env python3
"""Merge named-verb-guard into ~/.claude/settings.json Stop hooks. Idempotent."""
...
MARKER = "named-verb-guard/claude_stop_check.py"
EVENT = "Stop"
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude Stop named-verb-guard already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("added   claude Stop named-verb-guard")
