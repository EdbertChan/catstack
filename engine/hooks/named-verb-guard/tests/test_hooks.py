...
class TestTargetProof(unittest.TestCase):
    ...
    def test_on_hit_names_the_missing_piece(self):
        ...
        target = [job for job in captured if detect.TARGET_PROOF_REQUEST in job.get("hit_if_any_true", {})]
        self.assertEqual(len(target), 1)
        self.assertIn("Missing: the change ran before any check", target[0]["hit_if_any_true"][detect.TARGET_PROOF_REQUEST])
