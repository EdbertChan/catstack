...
def enqueue_judge(payload: dict) -> list[str]:
    ...
    hits = job.get("hit_if_any_true")
    if isinstance(hits, dict) and TARGET_PROOF_REQUEST in hits:
        hits[TARGET_PROOF_REQUEST] += " Missing: " + "; ".join(target_proof_gaps(message, tool_uses)) + "."
