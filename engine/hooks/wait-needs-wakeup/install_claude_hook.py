#!/usr/bin/env python3
"""Merge wait-needs-wakeup into ~/.claude/settings.json (PreToolUse on Bash
and Stop). Idempotent."""
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude PreToolUse+Stop wait-needs-wakeup already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("added   claude PreToolUse+Stop wait-needs-wakeup")
