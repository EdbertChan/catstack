#!/usr/bin/env python3
"""Merge reflect-on-thrash into ~/.claude/settings.json Stop hooks. Idempotent."""
...
MARKER = "reflect-on-thrash/claude_stop_reflect.py"
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude Stop reflect-on-thrash already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("link    claude Stop reflect-on-thrash merged into settings.json")
