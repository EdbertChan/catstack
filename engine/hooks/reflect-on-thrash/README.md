...
re-send, "you fucked up/messed up" aimed at the agent) reports immediately
through the shared hook runtime. In the registry's warn mode that is a warning;
a stop-mode override blocks through the harness's normal stop shape. That
prompt runs reflect **and** automate-me. Do not wait for session end or for the
user to re-prompt.
...
- `claude_stop_reflect.py` — Claude `Stop` (defer ordinary thrash; report intervention immediately)
- `codex_stop_reflect.py` — Codex `Stop` (same detector through the SDK runtime)
