...
MARKER = "unverified-tag-check/cursor_session.py"
COMMAND = "python3 $HOME/.cursor/hooks/unverified-tag-check/cursor_session.py"
...
def main() -> None:
    ...
    for key, incoming in (("stop", STOP_ENTRY), ("sessionEnd", SESSION_END_ENTRY)):
        ...
        if before != after:
            changed = True
            print(f"link    cursor {key} unverified-tag-check merged")
        else:
            print(f"ok      cursor {key} unverified-tag-check already up to date")
    if not changed:
        return
