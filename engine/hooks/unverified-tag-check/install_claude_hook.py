...
MARKER = "unverified-tag-check/claude_stop_check.py"
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude Stop unverified-tag-check already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("link    claude Stop unverified-tag-check merged into settings.json")
