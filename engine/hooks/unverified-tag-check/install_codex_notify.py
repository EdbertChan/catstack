...
MARKER = "unverified-tag-check/codex_notify.py"
CONFIG_PATH = os.path.expanduser("~/.codex/config.toml")
SCRIPT_PATH = os.path.expanduser("~/.codex/hooks/unverified-tag-check/codex_notify.py")
...
def compute_notify_update(config_text: str, script_path: str):
    ...
    if match:
        ...
        if any(MARKER in str(item) for item in current):
            return config_text, False, "codex notify unverified-tag-check already wired, skipping"
        ...
        return new_text, True, f"codex notify unverified-tag-check wired (chaining {len(current)} prior arg(s))"
    ...
    return new_text, True, "codex notify unverified-tag-check added (no prior notify command found)"
