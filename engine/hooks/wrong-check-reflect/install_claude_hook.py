#!/usr/bin/env python3
"""Merge wrong-check-reflect into ~/.claude/settings.json Stop hooks. Idempotent."""
...
MARKER = "wrong-check-reflect/claude_stop_check.py"
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude Stop wrong-check-reflect already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("link    claude Stop wrong-check-reflect merged into settings.json")
