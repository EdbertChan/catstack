...
A skill MUST be available to Claude, Cursor, and Codex unless it is listed in `CLAUDE_ONLY_SKILLS` in `install.sh`. Prefer catstack `product/skills/<name>/` or `corpus/skills/<name>/` + `./install.sh`. Project-skill home links MUST hit all three roots (`scripts/install/link_skill_three_harnesses.sh`). Do not follow Cursor-only `~/.cursor/skills/` install advice.
