...
- The `handback-needs-attempt` Stop hook fired (the assistant handed the user a command or step without attempting it, and no permission refusal or human-only blocker was present). Treat as FAIL; attempt the step first or name the real blocker, then present the finding when reflect runs.
...
- The user did a step by hand that the agent could have done: ran a command in their own terminal and pasted the output, edited a file themselves, pasted in the fix, looked something up, or said "I'll just do it." What the user did is the spec for what the agent should have done. Treat it as FAIL; the User-did-it lens ([references/lenses.md](references/lenses.md)) names each step and the agent action that would have replaced it. The `user-did-it` hook flags these live, through the llm-judge inbox. A step that physically needs the person (a password, hardware, an approval dialog, filming) is not this class.
...
3. **Repro gate (hard):** every detector/skill/hook change in the PR MUST include a positive synthetic fixture (fires) and a negative fixture (stays silent), with tests. Run `python3 scripts/ci/check_mine_repro_coverage.py` and `python3 scripts/ci/check_hook_test_coverage.py` when hooks change. Refuse to open the PR if either fails.
