...
scripts/ci/subagent_scope_debt_allowlist.txt, shrink-only in the same way as
scripts/ci/skill_test_debt_allowlist.txt. A new spawner must state the contract
rather than land on the list.

    python3 scripts/ci/check_subagent_scope_contract.py
    python3 scripts/ci/check_subagent_scope_contract.py --list   # spawners found
...
ALLOWLIST = REPO_ROOT / "scripts" / "ci" / "subagent_scope_debt_allowlist.txt"
