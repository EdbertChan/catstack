...
def check_repo() -> list[str]:
    ...
    link_script = os.path.join(REPO_ROOT, "scripts", "install", "link_skill_three_harnesses.sh")
    if not os.path.isfile(link_script):
        errors.append("scripts/install/link_skill_three_harnesses.sh missing")
    elif not os.access(link_script, os.X_OK):
        errors.append("scripts/install/link_skill_three_harnesses.sh is not executable")
