...
A skill listed in scripts/ci/skill_test_debt_allowlist.txt is grandfathered
...
Usage:
    python3 scripts/ci/check_skill_test_coverage.py            # check every skill
    python3 scripts/ci/check_skill_test_coverage.py --base <ref> [--head <ref>]
    python3 scripts/ci/check_skill_test_coverage.py --list-missing   # print all
...
ALLOWLIST_PATH = REPO_ROOT / "scripts" / "ci" / "skill_test_debt_allowlist.txt"
...
def main() -> int:
    ...
    if stale:
        print("fail  scripts/ci/skill_test_debt_allowlist.txt has stale entries (skill no longer exists):", file=sys.stderr)
        for rel in stale:
