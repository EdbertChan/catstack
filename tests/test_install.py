...
class TestCatModeDefaultInstall(unittest.TestCase):
    ...
    def test_unreadable_flag_file_is_named_as_unread_not_as_a_bad_value(self):
        """flags.py prints `unchecked` when a candidate .env cannot be read.
        Nobody typed that word, so install.sh must not report it as the value
        the user chose -- it says the flag could not be read, and falls back
        to off."""
        blocked = os.path.join(self.fake_home, "blocked.env")
        os.makedirs(blocked)
        result = run_install(self.fake_home, extra_env={"CATSTACK_ENV_FILE": blocked})
        output = result.stdout + result.stderr
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("CATSTACK_CAT_MODE_DEFAULT could not be read", output)
        self.assertNotIn("CATSTACK_CAT_MODE_DEFAULT=unchecked", output)
