...
class TestExemplars(unittest.TestCase):
    def test_catch_exemplars_are_superseded(self):
        for ex in gate.GATE_EXEMPLARS["catch"]:
            with self.subTest(ex=ex):
                self.assertTrue(gate.gate_check(ex))

    def test_allow_exemplars_are_live(self):
        for ex in gate.GATE_EXEMPLARS["allow"]:
            with self.subTest(ex=ex):
                self.assertFalse(gate.gate_check(ex))
                self.assertEqual(gate.classify(gate.Evidence(**ex))[0], gate.LIVE)
