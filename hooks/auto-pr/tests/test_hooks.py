...
def make_repo(tmp: str) -> str:
    ...
    os.makedirs(os.path.join(root, "hooks", "sample"))
    with open(os.path.join(root, "hooks", "sample", "detect.py"), "w", encoding="utf-8") as handle:
...
class TestRelevantPaths(unittest.TestCase):
    ...
    def test_relevant_paths_changed_detects_hook_edit(self):
        with open(os.path.join(self.repo, "hooks", "sample", "detect.py"), "a", encoding="utf-8") as handle:
            handle.write("# edit\n")
        self.assertIn("hooks/sample/detect.py", detect.relevant_paths_changed(self.repo))
