#!/usr/bin/env python3
"""Merge auto-pr into ~/.claude/settings.json Stop hooks. Idempotent."""
...
MARKER = "auto-pr/claude_stop_autopr.py"
...
def main() -> None:
    ...
    if not merge_hook(settings, fragment):
        print("ok      claude Stop auto-pr already up to date")
        return
    os.makedirs(os.path.dirname(SETTINGS_PATH), exist_ok=True)
    with open(SETTINGS_PATH, "w") as handle:
        json.dump(settings, handle, indent=2)
        handle.write("\n")
    print("link    claude Stop auto-pr merged into settings.json")
