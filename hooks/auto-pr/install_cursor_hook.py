#!/usr/bin/env python3
"""Merge auto-pr into ~/.cursor/hooks.json stop + sessionEnd. Idempotent."""
...
MARKER = "auto-pr/cursor_session.py"
COMMAND = "python3 $HOME/.cursor/hooks/auto-pr/cursor_session.py"
...
def main() -> None:
    ...
    for key, incoming in (("stop", STOP_ENTRY), ("sessionEnd", SESSION_END_ENTRY)):
        ...
        if before != after:
            changed = True
            print(f"link    cursor {key} auto-pr merged")
        else:
            print(f"ok      cursor {key} auto-pr already up to date")
    if not changed:
        return
