...
- Distinguish silent, agent-discovered drift (the failure mode this principle targets) from a user explicitly directing a sequence of related-but-expanding requests within one thread, each confirmed as it arrives. The latter is normal collaborative work, not scope creep — don't apply this principle just because a session covered a lot of ground; check whether the human was steering each expansion or whether the agent quietly wandered into it unasked.
